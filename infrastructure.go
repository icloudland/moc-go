...
func (r rawResponse) result() (result json.RawMessage, err error) {
...
func (c *Client) sendRequest(jReq *jsonRequest) {
	...
	url := protocol + "://" + c.config.Host + "/" + jReq.path
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := mocjson.CmdMethod(cmd)
	if err != nil {
	...
	if requestType == "POST" {
		marshalledJSON = []byte(queryString)
	}


	// Marshal the command.
	//marshalledJSON, err := mocjson.MarshalCmdSimple(cmd)
	//if err != nil {
	//	return newFutureError(err)
	//}
